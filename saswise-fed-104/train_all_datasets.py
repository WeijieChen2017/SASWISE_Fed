...
# Datasets to exclude from training (skip these datasets)
EXCLUDE_DATASETS = [
    ...
    with torch.no_grad():
        for inputs, targets in data_loader:
            ...
            if inputs.size(0) == 0 or targets.size(0) == 0:
                print(f"Warning: Skipping empty batch in evaluation. Input size: {inputs.size()}, Target size: {targets.size()}")
                continue
            ...
            if task == 'multi-label, binary-class':
                ...
                # Safe squeezing that preserves batch dimension
...
def train_and_evaluate(data_flag):
    ...
    print(f"Dataset: {data_flag}, Train samples: {len(train_dataset)}, Test samples: {len(test_dataset)}")
    print(f"Sample shape: {train_dataset[0][0].shape}, Label shape: {train_dataset[0][1].shape}")
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, targets in tqdm(train_loader, desc=f'Epoch {epoch+1}/{num_epochs} Training'):
            ...
            if inputs.size(0) == 0 or targets.size(0) == 0:
                print(f"Warning: Skipping empty batch {batch_idx}. Input size: {inputs.size()}, Target size: {targets.size()}")
                continue
            
            # Print batch info for first few batches in first epoch to aid debugging
            ...
            if task == 'multi-label, binary-class':
                ...
                if targets.numel() == 0:
                    print(f"Warning: Empty targets tensor found. Skipping batch.")
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exclude', type=str, nargs='+', default=[],
                      help='List of datasets to exclude from training')
    args = parser.parse_args()
    
    model_index = args.model_index
    
    # Add command-line excluded datasets to the exclude list
    ...
    for data_flag in selected_datasets:
        ...
        if data_flag in EXCLUDE_DATASETS:
            print(f"\n{'='*50}\nSkipping excluded dataset: {data_flag}\n{'='*50}")
            continue
        ...
        try:
            ...
            with open(f'errors_{model_index}.log', 'a') as f:
                f.write(f"\n\n{'='*50}\nError processing {data_flag}: {str(e)}\n")
